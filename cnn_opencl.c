...
static void tiled_convolution_layer(float* filters, float* biases, int d2, int d1, int n) {
	size_t global_size[] = { d1 * n * n * d2, PARALLEL };
	size_t local_size[] = { n * n ,  1 };

	err = clEnqueueWriteBuffer(queue, buf3, CL_TRUE, 0, sizeof(cl_float) * (d2 * d1 * 3 * 3), filters, 0, NULL, NULL);
