...
static void convolution_layer(float* inputs, float* outputs, float* filters, float* biases, int d1, int d2, int n) {
	...
	clEnqueueWriteBuffer(kernel_queue, buf3, CL_TRUE, 0, sizeof(cl_float) * (d2 * d1 * 3 * 3), filters, 0, NULL, NULL);
	clEnqueueWriteBuffer(kernel_queue, buf4, CL_TRUE, 0, sizeof(cl_float) * d2, biases, 0, NULL, NULL);

	clSetKernelArg(convolution_kernel_2, 1, sizeof(cl_mem), &buf3);
	clSetKernelArg(convolution_kernel_2, 2, sizeof(cl_float) * d1, NULL);
	clSetKernelArg(convolution_kernel_2, 3, sizeof(cl_mem), &buf2);
	clSetKernelArg(convolution_kernel_2, 4, sizeof(cl_mem), &buf4);
	clSetKernelArg(convolution_kernel_2, 5, sizeof(cl_int), &n);

	clSetKernelArg(convolution_kernel_22, 1, sizeof(cl_mem), &buf3);
	clSetKernelArg(convolution_kernel_22, 2, sizeof(cl_float) * d1, NULL);
	clSetKernelArg(convolution_kernel_22, 3, sizeof(cl_mem), &buf2_1);
	clSetKernelArg(convolution_kernel_22, 4, sizeof(cl_mem), &buf4);
	clSetKernelArg(convolution_kernel_22, 5, sizeof(cl_int), &n);
	...
	for (int i = 0; i < num_buffering; i += 2) {
		...
		float* input1 = inputs + i * batch_num * d1 * n * n;
		float* output1 = outputs + i * batch_num * d2 * n * n;
		clEnqueueWriteBuffer(kernel_queue, buf1, CL_TRUE, 0, sizeof(cl_float) * (batch_num * d1 * n * n), input1, 0, NULL, NULL);
		clSetKernelArg(convolution_kernel_2, 0, sizeof(cl_mem), &buf1);
		if (kernel_event[2] != NULL)
			clEnqueueNDRangeKernel(kernel_queue, convolution_kernel_2, 2, NULL, global_size, local_size, 1, &kernel_event[2], &kernel_event[0]);
		else
			clEnqueueNDRangeKernel(kernel_queue, convolution_kernel_2, 2, NULL, global_size, local_size, 0, NULL, &kernel_event[0]);
		clEnqueueReadBuffer(write_queue, buf2, CL_FALSE, 0, sizeof(cl_float) * (batch_num * d2 * n * n), output1, 1, &kernel_event[0], &kernel_event[1]);

		////////kernel2///////
		float* input2 = inputs + k * batch_num * d1 * n * n;
		float* output2 = outputs + k * batch_num * d2 * n * n;
		clEnqueueWriteBuffer(kernel_queue, buf1_1, CL_TRUE, 0, sizeof(cl_float) * (batch_num * d1 * n * n), input2, 0, NULL, NULL);
		clSetKernelArg(convolution_kernel_22, 0, sizeof(cl_mem), &buf1_1);
		clEnqueueNDRangeKernel(kernel_queue, convolution_kernel_22, 2, NULL, global_size, local_size, 1, &kernel_event[0], &kernel_event[2]);
		clEnqueueReadBuffer(write_queue, buf2_1, CL_FALSE, 0,
			sizeof(cl_float) * (batch_num * d2 * n * n), output2, 1, &kernel_event[2], &kernel_event[3]);
	}
...
static void pooling_layer(float* inputs, float* outputs, int d, int n) {
	...
	err = clSetKernelArg(pooling_kernel2, 1, sizeof(cl_mem), &buf2_1);
	err = clSetKernelArg(pooling_kernel2, 2, sizeof(cl_int), &n);
	...
	for (int i = 0; i < num_buffering; i += 2) {
		...
		clEnqueueWriteBuffer(kernel_queue, buf1, CL_TRUE, 0, sizeof(cl_float) * (batch_num * d * n * n * 4), input1, 0, NULL, NULL);
		clSetKernelArg(pooling_kernel, 0, sizeof(cl_mem), &buf1);
		if (kernel_event[2] != NULL)
			clEnqueueNDRangeKernel(kernel_queue, pooling_kernel, 2, NULL, global_size, NULL, 1, &kernel_event[2], &kernel_event[0]);
		else
			clEnqueueNDRangeKernel(kernel_queue, pooling_kernel, 2, NULL, global_size, NULL, 0, NULL, &kernel_event[0]);
		clEnqueueReadBuffer(write_queue, buf2, CL_FALSE, 0, sizeof(cl_float) * (batch_num * d * n * n), output1, 1, &kernel_event[0], &kernel_event[1]);

		////////kernel2///////
		float* input2 = inputs + k * batch_num * d * n * n * 4;
		float* output2 = outputs + k * batch_num * d * n * n;
		clEnqueueWriteBuffer(kernel_queue, buf1_1, CL_TRUE, 0, sizeof(cl_float) * (batch_num * d * n * n * 4), input2, 0, NULL, NULL);
		clSetKernelArg(pooling_kernel2, 0, sizeof(cl_mem), &buf1_1);
		clEnqueueNDRangeKernel(kernel_queue, pooling_kernel2, 2, NULL, global_size, NULL, 1, &kernel_event[0], &kernel_event[2]);
		clEnqueueReadBuffer(write_queue, buf2_1, CL_FALSE, 0,
			sizeof(cl_float) * (batch_num * d * n * n), output2, 1, &kernel_event[2], &kernel_event[3]);
	}
...
static void fc_layer(float* input_neuron, float* output_neuron, float* weights, float* biases, int N, int M) {
	...
	clSetKernelArg(fc_kernel, 0, sizeof(cl_mem), &buf2);
	...
	clSetKernelArg(fc_kernel, 5, sizeof(cl_int), &batch_num);  

	clSetKernelArg(fc_kernel2, 0, sizeof(cl_mem), &buf2_1);  
	clSetKernelArg(fc_kernel2, 2, sizeof(cl_mem), &buf3);   
	clSetKernelArg(fc_kernel2, 3, sizeof(cl_mem), &buf4);    
	clSetKernelArg(fc_kernel2, 4, sizeof(cl_float) * N, NULL);    
	clSetKernelArg(fc_kernel2, 5, sizeof(cl_int), &batch_num);   
	...
	for (int i = 0; i < num_buffering; i += 2) {
		...
		clEnqueueWriteBuffer(kernel_queue, buf1, CL_TRUE, 0, sizeof(cl_float) * (batch_num * N), input1, 0, NULL, NULL);  
		clSetKernelArg(fc_kernel, 1, sizeof(cl_mem), &buf1);    
		if (kernel_event[2] != NULL)
			clEnqueueNDRangeKernel(kernel_queue, fc_kernel, 2, NULL, global_size, local_size, 1, &kernel_event[2], &kernel_event[0]);
		else
			clEnqueueNDRangeKernel(kernel_queue, fc_kernel, 2, NULL, global_size, local_size, 0, NULL, &kernel_event[0]);      
		clEnqueueReadBuffer(write_queue, buf2, CL_FALSE, 0, sizeof(cl_float) * (batch_num * M), output1, 1, &kernel_event[0], &kernel_event[1]);

		////////kernel2///////
		float* input2 = input_neuron + k * batch_num * N;
		float* output2 = output_neuron + k * batch_num * M;
		clEnqueueWriteBuffer(kernel_queue, buf1_1, CL_TRUE, 0, sizeof(cl_float) * (batch_num * N), input2, 0, NULL, NULL);
		clSetKernelArg(fc_kernel2, 1, sizeof(cl_mem), &buf1_1);    CHECK_ERROR(err);
		clEnqueueNDRangeKernel(kernel_queue, fc_kernel2, 2, NULL, global_size, local_size, 1, &kernel_event[0], &kernel_event[2]);
		clEnqueueReadBuffer(write_queue, buf2_1, CL_FALSE, 0,
			sizeof(cl_float) * (batch_num * M), output2, 1, &kernel_event[2], &kernel_event[3]);
	}
